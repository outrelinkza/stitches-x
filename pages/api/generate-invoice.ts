...
async function generateInvoiceText(data: InvoiceData): Promise<string> {
...
function createInvoicePrompt(data: InvoiceData): string {
...
${additionalOptions.terms}`;
  ...
  switch (invoiceType) {
    case 'Freelancer / Service Invoice':
      prompt += `

This is a service invoice. Format it professionally with clear service descriptions and hourly rates.`;
      break;
    case 'Product / Sales Invoice':
      prompt += `

This is a product sales invoice. Format it with clear item descriptions, quantities, and unit prices.`;
      break;
    case 'Consulting / Agency Invoice':
      prompt += `

This is a consulting invoice. Format it professionally with detailed service descriptions and project information.`;
      break;
    case 'Simple Receipt':
      prompt += `

This is a simple receipt. Keep it minimal and clean with just the essential information.`;
      break;
    case 'Subscription / Recurring Invoice':
      prompt += `

This is a recurring subscription invoice. Include billing period and next billing date.`;
      break;
  }
...
async function generatePDF(invoiceText: string, data: InvoiceData): Promise<Buffer> {
  const doc = new jsPDF();
